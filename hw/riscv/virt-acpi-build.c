                       ...
                       RISCVVirtState *s)
{
    ...
    if (s->aia_type != VIRT_AIA_TYPE_NONE) {
        ...
        if ((s->aia_type == VIRT_AIA_TYPE_APLIC_IMSIC) && s->acpi_ged_msimode) {
            ...
            build_append_int_noprefix(table_data, 22, 1);   /* Length */
            build_append_int_noprefix(table_data, 1, 1);    /* Version */
            build_append_int_noprefix(table_data, 0, 1);    /* TODO: DO we need need it */
            build_append_int_noprefix(table_data, 0, 4);    /* Flags */
            build_append_int_noprefix(table_data, 0, 8);    /* ID */
            /* Total External Interrupt Sources Supported */
            build_append_int_noprefix(table_data, 4, 2);
            /* Global System Interrupt Base */
            build_append_int_noprefix(table_data, gsi_base, 4);
        }
    } else {
