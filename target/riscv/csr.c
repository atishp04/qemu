                                                         ...
                                                         bool upper_half)
{
    ...
    uint64_t *counter_arr_virt = icount_enabled() ? env->pmu_fixed_ctrs[1].counter_virt :
                            env->pmu_fixed_ctrs[0].counter_virt;
    uint64_t cfg_val = 0;

    if (counter_idx == 0) {
        cfg_val = upper_half ? ((uint64_t)env->mcyclecfgh << 32) : env->mcyclecfg;
    } else if (counter_idx == 2) {
        cfg_val = upper_half ? ((uint64_t)env->minstretcfgh << 32) : env->minstretcfg;
    } else {
        cfg_val = upper_half ? ((uint64_t)env->mhpmeventh_val[counter_idx] << 32) :
...
static int write_mhpmcounter(CPURISCVState *env, int csrno, target_ulong val)
{
    ...
    if (riscv_pmu_ctr_monitor_cycles(env, ctr_idx) ||
        ...
        counter->mhpmcounter_prev = riscv_pmu_ctr_get_fixed_counters_val(env, ctr_idx, false);
        if (ctr_idx > 2) {
            if (riscv_cpu_mxl(env) == MXL_RV32) {
...
static int write_mhpmcounterh(CPURISCVState *env, int csrno, target_ulong val)
{
    ...
    if (riscv_pmu_ctr_monitor_cycles(env, ctr_idx) ||
        riscv_pmu_ctr_monitor_instructions(env, ctr_idx)) {
        counter->mhpmcounterh_prev = riscv_pmu_ctr_get_fixed_counters_val(env, ctr_idx, true);
        if (ctr_idx > 2) {
            riscv_pmu_setup_timer(env, mhpmctr_val, ctr_idx);
        }
    } else {
                                         ...
                                         bool upper_half, uint32_t ctr_idx)
{
    ...
    if (riscv_pmu_ctr_monitor_cycles(env, ctr_idx) ||
        riscv_pmu_ctr_monitor_instructions(env, ctr_idx)) {
        *val = riscv_pmu_ctr_get_fixed_counters_val(env, ctr_idx, upper_half) - ctr_prev + ctr_val;
    } else {
        *val = ctr_val;
    }
